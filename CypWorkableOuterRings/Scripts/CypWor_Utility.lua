...
local RAILROAD_BOMB_TECH = GameInfo.Technologies[RAILROAD_BOMB_TECH_TYPE];
if RAILROAD_BOMB_TECH ~= nil then
  RAILROAD_BOMB_TECH_ID = RAILROAD_BOMB_TECH.Index;
end
-- Properties
...
CYP_WOR_DST_MIN = 4;
CYP_WOR_DST_MAX = 5;
CYP_WOR_DST_MAX_VAL = GameConfiguration.GetValue("CYP_WOR_CONFIG_RADIUS");
if CYP_WOR_DST_MAX_VAL ~= nil then
  CYP_WOR_DST_MAX = math.max(5, tonumber(CYP_WOR_DST_MAX_VAL));
end
-- Acquire plot
local CYP_WOR_GP_DUMMY_ACQUIRE_PLOT_TYPE = 'GREAT_PERSON_INDIVIDUAL_CYP_WOR_ACQUIRE_PLOT';
-- Properties (build on outer ring)
CYP_WOR_PROPERTY_OUTER_RING_BUILD_ADD_TO_QUEUE = "CYP_WOR_OUTER_RING_BUILD_ADD_TO_QUEUE";
CYP_WOR_PARAM_OUTER_RING_BUILD_ADD_TO_QUEUE = "CYP_WOR_OUTER_RING_BUILD_ADD_TO_QUEUE";
